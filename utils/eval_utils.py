...
def save_evaluation_curves(scores, labels, curves_save_path, video_frame_nums, n_score=0, auc_only=False, cal_marco=False):
    """
    ...
    if cal_marco:
        auroc_each_video = []
        # calc auc for each video
        for i in range(len(video_frame_nums)):
            cur_labels = np.empty(labels_each_video[i].shape)
            cur_labels[:] = labels_each_video[i][:]
            cur_scores = scores_each_video[i]
            if np.sum(labels_each_video[i])  > len(cur_labels) - 1:
                # print('fpr or tpr nan')
                cur_labels_list = list(cur_labels)
                cur_labels_list.append(0)
                cur_labels = np.array(cur_labels_list)
                cur_scores_list = list(cur_scores)
                cur_scores_list.append(n_score[len(cur_scores_list)])
                cur_scores = np.array(cur_scores_list)

            fpr, tpr, roc_thresholds = roc_curve(cur_labels, cur_scores, pos_label=1)
            auroc_each_video.append(auc(fpr, tpr))
